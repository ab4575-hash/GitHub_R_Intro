...
feet <- c(29031.7, 28251, 28169, 27940, 27838  )
  
meters <- c(8848,8611,8586,8516,8485)  
```

**2. Combine these vectors into a data frame called `mountains`.**
```{r}
mountains <- tibble (mountain, feet, meters) #combined vectors using the tibble function 
mountains
```
...
mean(mountains$feet)  #28,245.94 ft 
...
#view if vector was added to the dataset
glimpse(mountains)
```
...
```{r}
summits <- c(7563, 964, 312, 933, 891) #per individual
...
fatalities <- c(300, 92, 55, 50, 50)  #fatalities per mountain range 
